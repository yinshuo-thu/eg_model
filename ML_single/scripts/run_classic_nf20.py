...
def main():
    t0 = time.time()
    fcols = feature_cols()
    print(f"[classic-nf20] loading features ({len(fcols)} cols)", flush=True)
    df = load(fcols)
    tr = df["split"] == "train"
    ...
    print("[classic-nf20] Ridge", flush=True)
    rg = Ridge(alpha=200.0)
    rg.fit(Xtr, ytr)
    df["pred"] = rg.predict(Xall)
    r = evaluate(df, "ridge_nf20"); rows.append(r); print_row(r); save_pred(df, "ridge_nf20")

    print("[classic-nf20] ElasticNet", flush=True)
    en = ElasticNet(alpha=2e-3, l1_ratio=0.2, max_iter=3000)
    en.fit(Xtr, ytr)
    df["pred"] = en.predict(Xall)
    r = evaluate(df, "elasticnet_nf20"); rows.append(r); print_row(r); save_pred(df, "elasticnet_nf20")
    ...
    print("[classic-nf20] LightGBM L1 (bagged)", flush=True)
    df["pred"] = lgb_train(base, 600)
    r = evaluate(df, "lightgbm_l1_nf20"); rows.append(r); print_row(r); save_pred(df, "lightgbm_l1_nf20")

    print("[classic-nf20] LightGBM Huber (bagged)", flush=True)
    hub = dict(base); hub["objective"] = "huber"; hub["alpha"] = 0.9
    df["pred"] = lgb_train(hub, 600)
    r = evaluate(df, "lightgbm_huber_nf20"); rows.append(r); print_row(r); save_pred(df, "lightgbm_huber_nf20")

    print("[classic-nf20] LightGBM DART (bagged)", flush=True)
    dart = dict(base); dart["boosting"] = "dart"; dart["drop_rate"] = 0.1; dart["skip_drop"] = 0.5
    df["pred"] = lgb_train(dart, 500)
    r = evaluate(df, "lightgbm_dart_nf20"); rows.append(r); print_row(r); save_pred(df, "lightgbm_dart_nf20")

    lb = pd.DataFrame(rows)
    lb.to_csv(METR / "classic_nf20_leaderboard.csv", index=False)
    print(f"[classic-nf20] done in {time.time()-t0:.0f}s", flush=True)
