...
OUT_LB = ROOT / "ML_ensemble" / "metrics" / "ensemble_nf_leaderboard.csv"
OUT_PRED = PREDS / "ensemble_nf.parquet"
...
def load_bases():
    files = {f.stem: f for f in PREDS.glob("*_nf.parquet") if f.stem != "ensemble_nf"}
...
def main():
    ...
    print(f"[ens-nf] families: {fams}", flush=True)
    print(f"[ens-nf] blend members: {members}", flush=True)
    print(f"[ens-nf] valid IC per candidate: { {k: round(v,5) for k,v in valic.items()} }", flush=True)
    ...
    rows.append({"model": "ref_equal3_nf", **ic_ir(base, "m_equal"), "detail": f"equal {fam3}"})

    w_div3 = diversity_weights(base, fam3)
    base["m_div3"] = (base[fam3].to_numpy() * w_div3).sum(1)
    rows.append({"model": "div3_nf", **ic_ir(base, "m_div3"), "detail": f"w={wdict(fam3, w_div3)}"})

    w = diversity_weights(base, members)
    base["m_div"] = (M * w).sum(1)
    rows.append({"model": "divN_nf", **ic_ir(base, "m_div"), "detail": f"w={wdict(members, w)}"})
    ...
    print(f"[ens-nf] valid IC max {vmax:.5f} @plateau -> selected neutralise alpha = {alpha}", flush=True)

    base["pred"] = neutralize_g(base, "m_div", alpha)
    final = ic_ir(base, "pred")
    rows.append({"model": "FINAL ensemble_nf (divN+neutG)", **final,
    ...
    print(f"\n[ens-nf] FINAL  valid IC {final['valid_IC']:.5f} IR {final['valid_IR']:.3f} | "
          f"test IC {final['test_IC']:.5f} IR {final['test_IR']:.3f}", flush=True)

    print("\n[ens-nf] === comparison vs. existing baselines ===", flush=True)
    ...
    print(f"  {'ensemble_nf (213+new factors)':38s} valid IC {final['valid_IC']:.5f} IR {final['valid_IR']:.3f} | "
          f"test IC {final['test_IC']:.5f} IR {final['test_IR']:.3f}", flush=True)

    json.dump({"members": members, "weights": w.tolist(), "neut_alpha": alpha},
              open(ROOT / "ML_ensemble" / "models" / "ensemble_nf_config.json", "w"), indent=2)
