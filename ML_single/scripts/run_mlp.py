"""Multi-task tabular MLP for EG (DCN-style cross + deep tower, seed-ensembled).

Heads: main = y_xs (per-day z-scored return), aux = sign(y) (BCE) and |y_xs| (MSE)
as low-SNR regularisers. Early-stop on valid daily IC. GPU. Saves seed-averaged
valid+test predictions for the ensemble.
"""
...
def main():
    ...
    fcols = feature_cols()
    df = load(fcols)
    print(f"[mlp] {len(df):,} rows, {len(fcols)} feats, dev={DEV}", flush=True)
    ...
    df["pred"] = preds
    r = evaluate(df, "mlp"); print_row(r); save_pred(df, "mlp")
    pd.DataFrame([r]).to_csv(METR / "mlp_leaderboard.csv", index=False)
    print(f"[mlp] done in {time.time()-t0:.0f}s", flush=True)
