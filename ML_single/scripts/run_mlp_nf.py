...
output name -> mlp_nf, to isolate the lift from the new factors alone.
"""
...
def main():
    ...
    fcols = feature_cols()
    df = load(fcols)
    print(f"[mlp-nf] {len(df):,} rows, {len(fcols)} feats, dev={DEV}", flush=True)
    ...
    df["pred"] = preds
    r = evaluate(df, "mlp_nf"); print_row(r); save_pred(df, "mlp_nf")
    pd.DataFrame([r]).to_csv(METR / "mlp_nf_leaderboard.csv", index=False)
    print(f"[mlp-nf] done in {time.time()-t0:.0f}s", flush=True)
