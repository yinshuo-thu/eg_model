"""Shared utilities for the CURATED new-features (nf20) experiment: same
features_nf.parquet as common_nf.py, but restricted to feature_list_nf20.json
(213 original + top-20 new factors by |train IC| -- the noise-dilution ablation in
tools/ablate_topk_factors.py showed the full 111-factor block underperforms a small
curated subset for fixed-hyperparameter models)."""
...
FEAT = ROOT / "artifacts" / "features_nf.parquet"
...
def feature_cols() -> list[str]:
    return json.loads((ROOT / "artifacts" / "feature_list_nf20.json").read_text())
